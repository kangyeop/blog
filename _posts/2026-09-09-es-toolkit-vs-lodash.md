...
lodash는 여전히 프론트엔드 프로젝트에서 흔하게 보이는 유틸리티 라이브러리다. lodash 대안으로 es-toolkit이 언급되는 걸 본 지도 한참 됐는데, 트리쉐이킹이 더 잘된다는 정도만 알고 있었다. 그 외에 뭐가 다른지 궁금해서 찾아봤다.
...
es-toolkit을 트리쉐이킹이 더 잘되는 라이브러리로 이해하고 있었는데, 트리쉐이킹은 lodash-es도 된다. es-toolkit이 줄이는 건 함수 하나하나의 구현 크기였다.
...
함수 하나만 놓고 보면 대부분 90% 넘게 줄어든다. lodash-es 쪽 함수들이 이렇게 큰 이유는 오래된 브라우저 대응 코드 때문이다.
